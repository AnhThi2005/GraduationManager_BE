...
class CapNhatNguoiDungRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        $isTeacher = \App\Models\GiangVien::where('giang_vien_id', $id)->exists();
            ...
            $sv = \App\Models\SinhVien::where('ma_so_sinh_vien', $id)
    ...
    public function toServiceData(): array
    {
        ...
        if ($this->has('name')) $updateData['ho_ten'] = $this->input('name');
        if ($this->has('email')) $updateData['email'] = $this->input('email');
        if ($this->has('phone')) $updateData['so_dien_thoai'] = $this->input('phone');
        if ($this->has('gender')) $updateData['gioi_tinh'] = $this->input('gender');
        if ($this->has('dateOfBirth')) $updateData['ngay_sinh'] = $this->input('dateOfBirth');
        if ($this->has('status')) $updateData['dang_hoat_dong'] = $this->input('status') === 'active' ? 1 : 0;
        if ($this->has('className')) $updateData['className'] = $this->input('className');

        if ($isTeacher) {
            if ($this->has('academicDegree')) $updateData['hoc_vi'] = $this->input('academicDegree');
            if ($this->has('specialization')) $updateData['chuyen_mon'] = $this->input('specialization');
            if ($this->has('role')) $updateData['vai_tro'] = $this->input('role') === 'admin' ? 'ADMIN' : 'GIANG_VIEN';
        }
