...
class LichSuHoatDong extends Model
{
    ...
    public static function ghiLog($actionType, $description, $sinhVienId = null, $maSoSinhVien = null, $nhomId = null, $role = null, $userName = null, $details = null)
    {
        ...
        // 2. If nhomId is not provided, check details for dot_id or topic_id
        if (! $dotId && is_array($details)) {
            if (isset($details['dot_id'])) {
                $dotId = $details['dot_id'];
            } elseif (isset($details['topic_id'])) {
                $deTai = DeTai::find($details['topic_id']);
                if ($deTai) {
                    $dotId = $deTai->dot_id;
                }
            }
